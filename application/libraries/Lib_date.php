...
class Lib_date {
    ...
    public function deduct_time_from_datetime($datetime,$seconds) {
        if(is_valid_datetime($datetime)) {
            $time   = (int)convert_datetime_to_epoch($datetime)-(int)$seconds;
            $time   = convert_epoch_to_datetime($time);
            return $time;
        }
        return false;
    }
    ...
    // @param   Unix Timestamp  $when   Either 'now' or a valid unix timestamp.
    // *****************************************************************************
    public function get_formatted_date2( $when = 'now' ) {
        if( $when == 'now' ) {
            return date( 'l, jS  F Y' );
        }
        return date( 'l, jS F Y', $when );
    }

    // *****************************************************************************
    // Formats a timestamp with format ymm:
    // "0810"
    // -----
    // @param   Unix Timestamp  $when   Either 'now' or a valid unix timestamp.
    // *****************************************************************************
    public function get_formatted_date3( $when = 'now' ) {
        if( $when == 'now' ) {
            return date( 'ymm' );
        }
        return date( 'ymm', $when );
    }
